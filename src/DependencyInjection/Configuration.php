...
class Configuration implements ConfigurationInterface
{
        ...
        $rootNode
            ->children()
                ->arrayNode('logging')
                    ...
                    ->children()
                        ->scalarNode('application_name')
                            ...
                            ->isRequired()
                            ->validate()
                                ->ifTrue(fn($name): bool => !is_string($name))
                ...
                ->arrayNode('attach_request_id_injector_to')
                    ->prototype('scalar')
                    ->validate()
                        ->ifTrue(fn($serviceId): bool => !is_string($serviceId))
    ...
    private function createGatewayApiConfiguration(ArrayNodeDefinition $root): void
    {
        $root
            ->children()
                        ...
                        ->arrayNode('credentials')
                            ...
                            ->children()
                                ->scalarNode('username')
                                    ...
                                    ->isRequired()
                                    ->validate()
                                        ->ifTrue(fn($value): bool => !is_string($value) || $value === '')
                                        ->thenInvalid(
    ...
    private function createSmsConfiguration(ArrayNodeDefinition $root): void
    {
        $root
            ->children()
                ->arrayNode('sms')
                    ...
                    ->children()
                        ...
                        ->scalarNode('originator')
                            ...
                            ->validate()
                                ->ifTrue(fn($value): bool => !is_string($value) || !preg_match('~^[a-z0-9]{1,11}$~i', $value))
                                ->thenInvalid(
    ...
    private function createLocaleCookieConfiguration(ArrayNodeDefinition $root): void
    {
        $root
            ->children()
